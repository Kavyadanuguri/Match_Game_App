...
class App extends Component {
  ...
  render() {
    const {activeTabId, score, timer, randomNum, isTrue} = this.state

    const filteredList = imagesList.filter(
      each => each.category === activeTabId,
    )
    const {id, imageUrl} = imagesList[randomNum]
        ...
        <nav className="container1">
          <ul className="ul-nav">
            ...
            <li className="con1">
              ...
              />
              <p className="p2">{timer} Sec</p>
            </li>
          </ul>
        </nav>
        {!isTrue && (
          <div className="container2">
            <img alt="match" className="image3" src={imageUrl} />
            <ul className="ul-con1">
              {tabsList.map(each => (
                <li className="list-con1" key={each.tabId}>
                  <button
                    type="button"
                    className="button-tabItem "
                    onClick={() => this.onDisplayTab(each.tabId)}
                  >
                    <p
                      className={`text1 ${
                        activeTabId === each.tabId ? 'highlighted' : ''
                      }`}
                    >
                      {each.displayText}
                    </p>
            ...
            <ul className="container3">
              {filteredList.map(each => (
                <li className="list-con2" key={each.id}>
                  <button
                    type="button"
                    className="display-img-button"
                    onClick={() => this.onCheckImg(each.id, id)}
                  >
                    <img
                      alt="thumbnail"
        ...
        {isTrue && (
          <div className="container-con3">
            <div className="bg-image">
              ...
              <p className="score-p1">Your Score</p>
              <p className="score-p2">{score}</p>
              <button
                type="button"
                className="play-btn"
